    ...
    if duration is not None:
        with ffmpegio.open(video_file, 'rv', t=duration, r=fps) as f:
            for frame in f:
                ...
                channel.basic_publish(exchange='', routing_key=os.environ['RABBITMQ_QUEUE'], body=buffer.tobytes())
        connection.close()
        ...
        with ffmpegio.open(video_file, 'rv', r=fps) as f:
            for frame in f:
                ...
                channel.basic_publish(exchange='', routing_key=os.environ['RABBITMQ_QUEUE'], body=json.dumps(body))
        connection.close()
...
# vid2frame("rtsp://192.168.88.115:554/av0_0", 10)